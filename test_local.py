#!/usr/bin/env python3
"""
Test script for QuantAlert local setup (without Docker)
"""
...
def test_registration():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/register", json=data, timeout=10)
        if response.status_code == 200:
...
def test_login():
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/token", data=data, timeout=10)
        if response.status_code == 200:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if symbols:
                ...
                symbol = symbols[0]
                response = requests.get(f"{API_BASE}/price/{symbol}", timeout=10)
                if response.status_code == 200:
                ...
                # Test OHLCV endpoint
                response = requests.get(f"{API_BASE}/ohlcv/{symbol}?minutes=10", timeout=10)
                if response.status_code == 200:
...
def test_alerts(token):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE}/alerts", json=alert_data, headers=headers, timeout=10)
        if response.status_code == 200:
            alert = response.json()
            ...
            response = requests.get(f"{API_BASE}/alerts", headers=headers, timeout=10)
            if response.status_code == 200:
            ...
            response = requests.put(f"{API_BASE}/alerts/{alert_id}", json=update_data, headers=headers, timeout=10)
            if response.status_code == 200:
                print("✅ Alert updated successfully")
            
            # Test getting alert triggers
            response = requests.get(f"{API_BASE}/alerts/{alert_id}/triggers", headers=headers, timeout=10)
            if response.status_code == 200:
                triggers = response.json()
                print(f"✅ Alert triggers: {len(triggers)} triggers")
            
            # Test deleting alert
            response = requests.delete(f"{API_BASE}/alerts/{alert_id}", headers=headers, timeout=10)
            if response.status_code == 200:
...
def main():
    """Run all tests"""
    print("🚀 Starting QuantAlert Local Tests")
    print("=" * 50)
    ...
    print()
    print("🎯 Next steps:")
    print("1. Add your market data API keys to .env file")
    print("2. Restart the system to use real market data")
    print("3. Create alerts and watch for email notifications")
